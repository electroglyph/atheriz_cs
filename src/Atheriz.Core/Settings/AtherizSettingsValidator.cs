...
public sealed class AtherizSettingsValidator : IValidateOptions<AtherizSettings>
{
    ...
    public ValidateOptionsResult Validate(string? name, AtherizSettings options)
    {
        ...
        if (!IsValidInterface(options.WebserverInterface))
            failures.Add($"WebserverInterface unparseable: '{options.WebserverInterface}'.");
        if (!IsValidInterface(options.TelnetInterface))
        ...
        // surface crypto errors as config errors.
        ...
        if (options.TimeUpdateSeconds <= 0)
            failures.Add($"TimeUpdateSeconds must be >0 (was {options.TimeUpdateSeconds}).");
