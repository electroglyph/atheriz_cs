...
public class FuncParser
{
    ...
    public sealed class ParsedFunc
    {
        ...
        // Raw source echo: FullStr holds `$name(` plus the executed text,
        // InFuncStr is unused at execution time (kept for the echo shape).
        ...
        FuncParserCallables = new Dictionary<string, ParserCallable>(StringComparer.Ordinal)
        {
            ...
            ["choice"] = (a,k,ctx,raw) => { if(a.Length==0) return ""; var rnd=Random.Shared;
                if(a.Length==1){ var single=a[0].Trim(); if(single.StartsWith("[", StringComparison.Ordinal)&&single.EndsWith("]", StringComparison.Ordinal)){ try{ var inner=single.Substring(1,single.Length-2); var items=inner.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(s=>s.Trim()).ToArray(); if(items.Length>0) return items[rnd.Next(items.Length)].Trim('\'','"'); }catch (Exception logEx) { AtherizLogger.LogDebug("Suppressed ParsedFunc.Choice: " + logEx.Message, "ParsedFunc"); } } try{
                    ...
                    var converters = Enumerable.Repeat(FuncParserHelpers.PyConverter, a.Length).ToArray();
                    var conv = FuncParserHelpers.SafeConvertToTypes(converters, a.Cast<object?>().ToArray(), new Dictionary<string,object?>(), raiseErrors: ctx.RaiseErrors);
    ...
    private static string SafePyEval(string s)
    {
        ...
        try{
            var conv = FuncParserHelpers.SafeConvertToTypes([FuncParserHelpers.PyConverter], new object?[]{s}, new Dictionary<string,object?>(), raiseErrors: true);
            var v = conv.args[0];
    ...
    // Constructors. Each public ctor copies its table (later caller-side
    // mutation must not rewire a live parser) and stores the scalars.
    // Game callables use the ParserCallable shape directly — explicit
    // lambdas, no signature sniffing.
    ...
    // Single engine behind the public Execute and the parser loop below.
    public object? Execute(ParsedFunc pf, bool raiseErrors = false, IDictionary<string, object?>? reservedKwargs = null)
        => ExecuteCall(_callables, _defaultKwargs, this, pf, raiseErrors, reservedKwargs);

    internal static object? ExecuteCall(IReadOnlyDictionary<string, ParserCallable> callables, IReadOnlyDictionary<string, object?> defaultKwargs, FuncParser? owner, ParsedFunc pf, bool raiseErrors, IDictionary<string, object?>? reservedKwargs)
    {
        var funcname = pf.FuncName;
        if (!callables.TryGetValue(funcname, out var func))
        {
            if(raiseErrors) throw new ParsingError($"Unknown parsed function '{pf}' (available: {string.Join(", ", callables.Keys.Select(k=>"'"+k+"'"))})");
            return pf.ToString();
        }
    ...
    // instance); the same reference is threaded into merged kwargs so
    ...
    private sealed class RecursiveParser
    {
        ...
        void ParseTopCall()
        {
            ...
            pos++; // consume $
            var (closed, _, echo) = ParseFrame(frame, 1);
        ...
        (bool Closed, object? Value, string Echo) ParseFrame(Frame f, int depth)
        {
            int n = str.Length;
            bool escaped = false;
            while (pos < n)
            {
                ...
                if (ch == startChar && f.Quoted < 0)
                {
                    ...
                    var (closed, value, echo) = ParseFrame(child, depth + 1);
                ...
                if (ch == ',' || ch == ')')
                {
                    ...
                    if (ch == ')')
                    {
                        ...
                        f.Paren = 0;
                        pos++;
                        object? exec = stripMode ? (object?)"" : escapeMode ? (object?)(escapeChar + f.Echo.ToString()) : ExecuteCall(callables, defaultKwargs, owner, ToParsedFunc(f), raiseErrors, reservedKwargs);
        ...
        // `,` or a call-level `)`: the pending nested result or the
        // accumulated text becomes the next arg/kwarg value. A pending
        // nested result keeps its raw object for positional args;
        // everything else stringifies here. The echo records the executed
        // value plus the raw text, so an unclosed parent echoes faithfully.
        static void FinalizeArg(Frame f, char ch)
        {
        ...
        ParsedFunc ToParsedFunc(Frame f)
        {
