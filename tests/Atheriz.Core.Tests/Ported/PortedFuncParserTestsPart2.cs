...
public class PortedFuncParserTestsPart2
{
    ...
    [Fact] public void EvalLiteralInt(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("42", p.Parse("$eval(42)")?.ToString());}
    [Fact] public void EvalLiteralList(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("$eval([1,2,3])")?.ToString(); Assert.True(r!.Contains("1") && r.Contains("2")); }
    [Fact] public void EvalLiteralString(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("hello", p.Parse("$eval('hello')")?.ToString());}
    [Fact] public void EvalEmpty(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("", p.Parse("$eval()")?.ToString());}
    [Fact] public void Add(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("7", p.Parse("$add(3,4)")?.ToString());}
    [Fact] public void Sub(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("7", p.Parse("$sub(10,3)")?.ToString());}
    [Fact] public void Mult(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("12", p.Parse("$mult(3,4)")?.ToString());}
    [Fact] public void Div(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("5", p.Parse("$div(10,2)")?.ToString());}
    [Fact] public void AddTooFew(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("", p.Parse("$add(3)")?.ToString());}
    [Fact] public void Round(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("4", p.Parse("$round(3.7)")?.ToString()); Assert.Equal("3.54", p.Parse("$round(3.54343,2)")?.ToString()); Assert.Equal("", p.Parse("$round()")?.ToString());}
    [Fact] public void ToInt(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("43", p.Parse("$toint(43.0)")?.ToString()); Assert.Equal("42", p.Parse("$toint(42)")?.ToString()); Assert.Equal("abc", p.Parse("$toint(abc)")?.ToString());}
    [Fact] public void RandomInRange(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("$random(5,10)")?.ToString(); Assert.True(int.TryParse(r, out var v) && v>=5 && v<=10); var rf=p.Parse("$random(5.0,10)")?.ToString(); Assert.True(double.TryParse(rf, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out _)); Assert.True(rf!.Contains('.')||rf.Contains(','));}
    [Fact] public void RandomNoArgs(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("$random()")?.ToString(); Assert.True(r=="0"||r=="1"); }
    [Fact] public void RandintAlwaysInt(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("$randint(5.0,10.0)")?.ToString(); Assert.True(int.TryParse(r, out _)); }
    [Fact] public void ChoiceFromList(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("$choice([1, 2, 3])")?.ToString(); Assert.Contains(r, new[]{"1","2","3"});}
    [Fact] public void ChoiceFromArgs(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); // docstring: choice(a,b,c) with non-literal strings raises ParsingError when raiseErrors true
        Assert.Throws<FuncParser.ParsingError>(()=> p.Parse("$choice(a,b,c)", raiseErrors:true));
    }
    [Fact] public void ChoiceFromIntArgs(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("$choice(1,2,3)")?.ToString(); Assert.Contains(r, new[]{"1","2","3"});}
    [Fact] public void ChoiceNoArgs(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("", p.Parse("$choice()")?.ToString());}
    [Fact] public void Pad(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("$pad(hi,10,c)")?.ToString(); Assert.Equal(10, r!.Length); Assert.Contains("hi", r); var lr=p.Parse("$pad(hi,10,l)")?.ToString(); Assert.StartsWith("hi", lr); var rr=p.Parse("$pad(hi,10,r)")?.ToString(); Assert.EndsWith("hi", rr);}
    [Fact] public void PadInvalidAlignDefaultsToCenter(){ using var env=GlobalTestEnv.Enter(); var result=new FuncParser(FuncParser.FuncParserCallables).Parse("$pad(hi,10,x)")?.ToString(); Assert.Equal(10, result!.Length); Assert.Equal((10-2)/2, result.IndexOf("hi"));}
    [Fact] public void PadNoArgs(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("", p.Parse("$pad()")?.ToString());}
    [Fact] public void Crop(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("hi", p.Parse("$crop(hi,10)")?.ToString()); var r=p.Parse("$crop("+new string('a',100)+",10,...)")?.ToString(); Assert.Equal(10, r!.Length); Assert.EndsWith("...", r);}
    [Fact] public void CropCustomSuffix(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("$crop("+new string('a',100)+",10,X)")?.ToString(); Assert.EndsWith("X", r);}
    [Fact] public void Justify(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var l=p.Parse("$justify(hi,10,l)")?.ToString(); Assert.StartsWith("hi", l); var r=p.Parse("$justify(hi,10,r)")?.ToString(); Assert.EndsWith("hi", r); var c=p.Parse("$justify(hi,10,c)")?.ToString(); Assert.Contains("hi", c); Assert.Equal(10, c!.Split('\n')[0].Length);}
    [Fact] public void JustifyLegacyLeft(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("$ljust(hi,10)")?.ToString(); Assert.Equal(10, r!.Length); var r2=p.Parse("$rjust(hi,10)")?.ToString(); Assert.Equal(10, r2!.Length); var r3=p.Parse("$cjust(hi,10)")?.ToString(); Assert.Equal(10, r3!.Length);}
    [Fact] public void Space(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("     ", p.Parse("$space(5)")?.ToString()); Assert.Equal("", p.Parse("$space()")?.ToString()); Assert.Equal(" ", p.Parse("$space(abc)")?.ToString());}
    [Fact] public void Clr(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("|rtext|n", p.Parse("$clr(r,text,n)")?.ToString()); Assert.Equal("|rtext|n", p.Parse("$clr(r,text)")?.ToString()); Assert.Equal("text", p.Parse("$clr(text)")?.ToString()); Assert.Equal("|rtext|n", p.Parse("$clr(text, start=r, end=n)")?.ToString());}
    [Fact] public void Pluralize(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("cat", p.Parse("$pluralize(cat,1)")?.ToString()); Assert.Equal("cats", p.Parse("$pluralize(cat,2)")?.ToString()); Assert.Equal("cat", p.Parse("$pluralize(cat,0)")?.ToString()); Assert.Equal("geese", p.Parse("$pluralize(goose,3,geese)")?.ToString()); Assert.Equal("", p.Parse("$pluralize()")?.ToString());}
    [Fact] public void Int2Str(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("one", p.Parse("$int2str(1)")?.ToString()); Assert.Equal("twelve", p.Parse("$int2str(12)")?.ToString()); Assert.Equal("15", p.Parse("$int2str(15)")?.ToString()); Assert.Equal("no", p.Parse("$int2str(0)")?.ToString());}
    [Fact] public void An(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("an apple", p.Parse("$an(apple)")?.ToString()); Assert.Equal("a banana", p.Parse("$an(banana)")?.ToString()); Assert.Equal("an yellow", p.Parse("$an(yellow)")?.ToString());}
    [Fact] public void AnNoArgs(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("", p.Parse("$an()")?.ToString());}
    // Actor stance
    [Fact] public void YouSelf(){ using var env=GlobalTestEnv.Enter(); var hero=GameObject.Create("Hero"); var p=new FuncParser(FuncParser.ActorStanceCallables); var r=p.Parse("$you()", hero, hero, null)?.ToString(); Assert.Equal("you", r); var r2=p.Parse("$You()", hero, hero, null)?.ToString(); Assert.Equal("You", r2); }
    [Fact] public void YouOther(){ using var env=GlobalTestEnv.Enter(); var hero=GameObject.Create("Hero"); var villain=GameObject.Create("Villain"); ObjectRegistry.AddObject(hero); ObjectRegistry.AddObject(villain); var p=new FuncParser(FuncParser.ActorStanceCallables); var r=p.Parse("$you()", hero, villain, null)?.ToString(); Assert.Equal("Hero", r); }
    [Fact] public void YouNoCallerOrReceiverRaises(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.ActorStanceCallables); var ex=Assert.Throws<FuncParser.ParsingError>(()=> p.Parse("$you()", raiseErrors:true)); Assert.Contains("No caller", ex.Message); }
    [Fact] public void YourSelf(){ using var env=GlobalTestEnv.Enter(); var hero=GameObject.Create("Hero"); var p=new FuncParser(FuncParser.ActorStanceCallables); Assert.Equal("your", p.Parse("$your()", hero, hero, null)?.ToString()); Assert.Equal("Your", p.Parse("$Your()", hero, hero, null)?.ToString());}
    [Fact] public void YourNoCallerRaises(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.ActorStanceCallables); var ex=Assert.Throws<FuncParser.ParsingError>(()=> p.Parse("$your()", raiseErrors:true)); Assert.Contains("No caller", ex.Message);}
    [Fact] public void ConjugateSelfVsOther(){ using var env=GlobalTestEnv.Enter(); var alice=GameObject.Create("Alice"); var bob=GameObject.Create("Bob"); var p=new FuncParser(FuncParser.ActorStanceCallables); Assert.Equal("jump", p.Parse("$conj(jump)", alice, alice, null)?.ToString()); Assert.Equal("jumps", p.Parse("$conj(jump)", alice, bob, null)?.ToString());}
    [Fact] public void ConjugateNoCallerRaises(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.ActorStanceCallables); var ex=Assert.Throws<FuncParser.ParsingError>(()=> p.Parse("$conj(jump)", raiseErrors:true)); Assert.Contains("No caller", ex.Message);}
    [Fact] public void ConjugateNoArgs(){ using var env=GlobalTestEnv.Enter(); var alice=GameObject.Create("Alice"); var p=new FuncParser(FuncParser.ActorStanceCallables); Assert.Equal("", p.Parse("$conj()", alice, alice, null)?.ToString());}
    [Fact] public void ConjugateWithMapping(){ using var env=GlobalTestEnv.Enter(); var alice=GameObject.Create("Alice"); var mapping=new Dictionary<string,object?>{["tommy"]=alice}; var p=new FuncParser(FuncParser.ActorStanceCallables); Assert.Equal("jump", p.Parse("$conj(jump, tommy)", alice, alice, mapping)?.ToString());}
    [Fact] public void PConjSelf(){ using var env=GlobalTestEnv.Enter(); var alice=GameObject.Create("Alice"); var p=new FuncParser(FuncParser.ActorStanceCallables); Assert.Equal("jump", p.Parse("$pconj(jump)", alice, alice, null)?.ToString());}
    [Fact] public void PConjOtherSingular(){ using var env=GlobalTestEnv.Enter(); var male=GameObject.Create("Male"); male.Gender="male"; var other=GameObject.Create("Other"); var p=new FuncParser(FuncParser.ActorStanceCallables); Assert.Equal("jumps", p.Parse("$pconj(jump)", male, other, null)?.ToString());}
    [Fact] public void PConjOtherPlural(){ using var env=GlobalTestEnv.Enter(); var plural=GameObject.Create("Plural"); plural.Gender="plural"; var other=GameObject.Create("Other"); var p=new FuncParser(FuncParser.ActorStanceCallables); var r=p.Parse("$pconj(jump)", plural, other, null)?.ToString(); Assert.Contains("jump", r!); // plural keeps base
    }
    [Fact] public void Pronoun(){ using var env=GlobalTestEnv.Enter(); var male=GameObject.Create("Bob"); male.Gender="male"; var p=new FuncParser(FuncParser.ActorStanceCallables); Assert.Equal("I", p.Parse("$pron(I)", male, male, null)?.ToString()); Assert.Equal("he", p.Parse("$pron(I)", male, GameObject.Create("Other"), null)?.ToString());}
    [Fact] public void PronounOtherPlural(){ using var env=GlobalTestEnv.Enter(); var plural=GameObject.Create("Plural"); plural.Gender="plural"; var other=GameObject.Create("Other"); var p=new FuncParser(FuncParser.ActorStanceCallables); var r=p.Parse("$pron(I)", plural, other, null)?.ToString(); Assert.Equal("they", r); }
    ...
    // Integration
    [Fact] public void IntegrationBuiltIn(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); Assert.Equal("an apple and a banana", p.Parse("$an(apple) and $an(banana)")?.ToString());}
    [Fact] public void IntegrationActorStance(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.ActorStanceCallables);
    ...
    [Fact] public void ComplexString(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("Count: $eval(1+2), Plural: $pluralize(cat, 3), An: $an(orange)")?.ToString(); Assert.Contains("Count: 3", r!); Assert.Contains("Plural: cats", r!); Assert.Contains("An: an orange", r!);}
    [Fact] public void SafeFailureOneFunc(){ using var env=GlobalTestEnv.Enter(); var p=new FuncParser(FuncParser.FuncParserCallables); var r=p.Parse("A $eval(abc)B and $an(orange)")?.ToString(); Assert.Contains("an orange", r!); Assert.IsType<string>(r); Assert.StartsWith("A", r); Assert.Contains("B and", r!);}
    ...
    [Fact] public void ParsedFuncDefaultsAreLists(){
        ...
        Assert.IsType<StringBuilder>(pf.FullStr);
        Assert.IsType<StringBuilder>(pf.InFuncStr);
    ...
    [Fact] public void ParsedFuncStrHandlesBoth(){
        using var env=GlobalTestEnv.Enter();
        var pf1=new FuncParser.ParsedFunc('$'); pf1.FullStr.Clear(); pf1.FullStr.Append("$foo("); pf1.InFuncStr.Append("bar");
        Assert.Equal("$foo(bar", pf1.ToString());
        var pf2=new FuncParser.ParsedFunc(); pf2.FullStr.Append("$foo("); pf2.InFuncStr.Append("bar");
