...
// `#id` parsing has one truth: malformed references ("#", "#x", "#-1",
// "#+12", "# 123") report the invalid-format message while
// well-formed-but-missing ids ("#99999") report not-found. Both id
// consumers share the parse.
[Collection("Ported")]
public sealed class IdRefResolutionTests
{
    ...
    public void TryParseIdRef_EdgeInputs()
    {
        ...
        Assert.Equal(5, id);
        Assert.False(CommandHelpers.TryParseIdRef("#-1", out _));
        Assert.False(CommandHelpers.TryParseIdRef("# 123", out _));
        Assert.False(CommandHelpers.TryParseIdRef("#+12", out _));
    ...
    public void ResolveObject_MalformedId_ReportsInvalidFormat()
    {
        ...
        foreach (var q in new[] { "#", "#x", "#-1", "#+12", "# 123" })
        {
