// Pins for simplify3 §3 batch B18 (C2 remainder of the consolidated GetSingle
// sweep): the five single-fetch sites that still allocated a list via
...
[Collection("Ported")]
public sealed class B18ConsolidatedSingleFetchTests
{
    private sealed class B18ProbeScript : Script
    {
        [Before]
        public void at_b18_probe() { }
    }
    ...
    public void AtMapUpdate_ObjectLocationOfNode_ResolvesNodeCoord()
    {
        ...
        var node = new Node(new Coord("b18map", 12, 18, 0));
    ...
    public void AddScript_ByIdResolves_InstallsHooks()
    {
        ...
        var script = new B18ProbeScript();
        ObjectRegistry.AddObject(child);
        ObjectRegistry.AddObject(script);

        child.AddScript(script.Id);

        Assert.True(child.HasHook("at_b18_probe"));
        Assert.True(child.HasScriptType("B18Probe"));
    ...
    public void AddScript_MissingId_SilentlySkips()
    {
        ...
        Assert.Null(ex);
        Assert.False(child.HasHook("at_b18_probe"));
    ...
    public void AddScript_WrongTypeId_SilentlySkips()
    {
        ...
        Assert.Null(ex);
        Assert.False(child.HasHook("at_b18_probe"));
        Assert.Empty(child.GetScriptsByType("B18Probe"));
    }
    ...
    public void HasScriptType_MissingId_ReturnsFalse()
    {
        ...
        Assert.False(child.HasScriptType("b18probe"));
    }
