// Pins for U11 (GameArgumentParser version-counter invalidation): repeated parses
...
[Collection("Ported")]
public sealed class B25bU11DefMapsTests
{
    [Fact]
    public void ParseArgs_RepeatedParses_SameShape()
    {
        var p = new GameArgumentParser("b25b_u11_shape");
        p.AddArgument("name");
        p.AddArgument("--tag");
    ...
    {
        var p = new GameArgumentParser("b25b_u11_add");
        p.AddArgument("first");
    ...
    {
        var p = new GameArgumentParser("b25b_u11_builder");
